    ...
    <div class="page-container">
        <div class="content-wrapper">
            ...
            <div class="page-header">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    {{-- <a href="{{route('orders.create')}}" class="btn btn-primary">
                        <i class="ri-add-line me-2"></i>สร้างคำสั่งซื้อ
                    </a> --}}  
                </div>
            </div>
            ...
            <div class="stats-grid">
                ...
                <div class="stat-card">
                    <div class="stat-content">
                        <h3>฿{{ number_format($orders->sum('order_grand_total'), 0) }}</h3>
                ...
                <div class="table-header">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div class="text-muted" style="font-size: 14px;">
                            @if($search)
                                ค้นหา: "<strong>{{ $search }}</strong>"
                            @endif
                            @if($status)
                                | สถานะ: <strong>
                                    @if($status === 'pending') รอดำเนินการ
                                    @elseif($status === 'success') สำเร็จ
                                    @elseif($status === 'cancel') ยกเลิก
                                    @endif
                                </strong>
                            @endif
                            | พบ {{ $orders->total() }} รายการ
                        </div>
                        @endif
                    </div>
                </div>
                    ...
                    <tbody>
                        @forelse ($orders as $order)
                            <tr>
                                ...
                                <td>
                                    <div>
                                        <div style="font-weight: 600; color: #047857;">{{ $order->order_date->format('d/m/Y') }}</div>
                                        <div style="font-size: 12px; color: #6b7280;">{{ $order->order_date->format('H:i น.') }}</div>
                                    </div>
                                </td>
                                <td>
                                    <div class="customer-info">
                                        <div class="customer-name">{{ $order->customer->customer_name ?? '-' }}</div>
                                        <div class="customer-phone">{{ $order->customer->customer_phone ?? '-' }}</div>
                                        <div class="customer-address" title="{{ $order->customer->customer_address ?? '-' }}">
                                ...
                                <td>
                                    <div style="font-size: 13px; color: #374151; max-width: 180px;">
                                        <div style="font-weight: 600; color: #059669;">
                                            {{ optional($order->deliveryAddress)->delivery_contact_name ?? $order->customer->customer_name }}
                                        </div>
                                        <div style="font-size: 12px; color: #6b7280; margin-top: 2px;">
                                            {{ optional($order->deliveryAddress)->delivery_phone ?? $order->customer->customer_phone }}
                                        </div>
                                        <div style="font-size: 12px; color: #6ee7b7; margin-top: 2px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" 
                                             title="{{ optional($order->deliveryAddress)->delivery_address ?? $order->customer->customer_address }}">
                                            {{ optional($order->deliveryAddress)->delivery_address ?? $order->customer->customer_address }}
                                        </div>
                                    </div>
                                </td>
                                <td style="text-align: right;">
                                    <span class="amount">฿{{ number_format($order->order_grand_total, 2) }}</span>
                                </td>
                                <td>
                                    ...
                                    @if($order->payment_status === 'pending' || $order->payment_status === 'partial' || $order->payment_status === 'waiting_confirmation')
                                        <a href="{{ route('orders.payment.livewire', $order->id) }}" class="btn btn-sm btn-primary mt-2">แจ้งชำระเงิน</a>
                                    @endif
                                </td>
                ...
                <div class="pagination-wrapper">
                    <div class="d-flex justify-content-between align-items-center">
                        <div style="color: #6b7280; font-size: 14px;">
                            แสดง {{ $orders->firstItem() }}-{{ $orders->lastItem() }} จาก {{ $orders->total() }} รายการ
                        </div>
                        <div>
