...
// Import Facades ต้องอยู่ด้านบนของไฟล์
...
try {
    ...
    try {
        ...
        foreach ($order->items as $item) {
            ...
            if ($remaining > 0) {
                // ดึงข้อมูลการคำนวณจาก order_item
                $productCalc = $item->product_calculation;
                $unitPrice = $item->unit_price;
                $total = $unitPrice * $remaining;
                
                // ใช้ create แทน new เพื่อใช้ mass assignment
                \App\Models\Orders\OrderDeliveryItems::create([
                    'order_delivery_id' => $delivery->id,
                    'order_item_id' => $item->id,
                    'quantity' => $remaining, // จัดส่งทั้งหมดที่เหลือ
                    'unit_price' => $unitPrice,
                    'product_calculation' => $productCalc, // เพิ่มข้อมูลการคำนวณสินค้า
                    'total' => $total
                ]);
                
                // คำนวณน้ำหนัก
                $productWeight = $item->product_weight ?? 0; // ใช้ค่าจาก order_item โดยตรง
