        ...
        <div class="col-md-3">
            <div class="card text-white bg-primary" style="border-radius: 12px; box-shadow: 0 4px 20px rgba(102, 126, 234, 0.15);">
                <div class="card-body">
                    ...
                    <p class="card-text fs-4" style="font-size:2rem;"><?php echo e($totalOrders); ?></p>
                </div>
            </div>
        </div>
        <div class="col-md-3">
                ...
                <tbody>
                    <!--[if BLOCK]><![endif]--><?php $__empty_1 = true; $__currentLoopData = $orders; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $order): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr>
                            ...
                            <td><span style="font-weight:700; color:#3b82f6; background:linear-gradient(135deg,#dbeafe 0%,#bfdbfe 100%); padding:6px 12px; border-radius:6px; border:1px solid #93c5fd;"><?php echo e($order->order_number); ?></span></td>
                            <td>
                                <div style="font-weight:600; color:#111827;"><?php echo e($order->order_date->format('d/m/Y')); ?></div>
                                <div style="font-size:12px; color:#6b7280;"><?php echo e($order->order_date->format('H:i น.')); ?></div>
                            </td>
                            <td>
                                <div style="font-weight:600; color:#111827;"><?php echo e($order->customer->customer_name ?? '-'); ?></div>
                                <div style="font-size:13px; color:#6b7280; background:#f3f4f6; padding:2px 8px; border-radius:4px; display:inline-block;"><?php echo e($order->customer->customer_phone ?? '-'); ?></div>
                                <div style="font-size:12px; color:#9ca3af; margin-top:4px; max-width:200px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;" title="<?php echo e($order->customer->customer_address ?? '-'); ?>"><?php echo e($order->customer->customer_address ?? '-'); ?></div>
                            </td>
                            <td>
                                <div style="font-size:13px; color:#374151; max-width:180px;">
                                    <div style="font-weight:600; color:#059669;"><?php echo e(optional($order->deliveryAddress)->delivery_contact_name ?? $order->customer->customer_name); ?></div>
                                    <div style="font-size:12px; color:#6b7280; margin-top:2px;"><?php echo e(optional($order->deliveryAddress)->delivery_phone ?? $order->customer->customer_phone); ?></div>
                                    <div style="font-size:12px; color:#9ca3af; margin-top:2px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;" title="<?php echo e(optional($order->deliveryAddress)->delivery_address ?? $order->customer->customer_address); ?>"><?php echo e(optional($order->deliveryAddress)->delivery_address ?? $order->customer->customer_address); ?></div>
                                </div>
                            </td>
                            <td><span style="font-weight:700; color:#059669; font-size:17px; background:linear-gradient(135deg,#d1fae5 0%,#a7f3d0 100%); padding:8px 16px; border-radius:8px; border:1px solid #6ee7b7; display:inline-block;"><?php echo e(number_format($order->order_grand_total, 2)); ?></span></td>
                            <td>
