                    ...
                    <div class="form-grid-date">
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control"
                                   ...
                                   <?php echo e($quote_status === 'cancel' && !auth()->user()->hasAdminAccess() ? 'disabled' : ''); ?>
                        ...
                        <!--[if BLOCK]><![endif]--><?php if($quotation && $quote_status === 'wait'): ?>
                            <div class="d-flex gap-2">
                                ...
                                <button type="button" class="btn btn-outline-danger"
                                        onclick="showRejectReasonModal(<?php echo e($quotation->id); ?>, '<?php echo e($quotation->quote_number); ?>')">
                                    <i class="ri-close-line me-2"></i>ไม่อนุมัติใบเสนอราคา
                                </button>
                            </div>
                        <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
                        <!--[if BLOCK]><![endif]--><?php if($quotation && $quote_status === 'cancel'): ?>
                            <div class="d-flex gap-2">
                                <button type="button" class="btn btn-primary"
                                        wire:click="revertToWaitStatus(<?php echo e($quotation->id); ?>)"
                                        onclick="return confirm('ต้องการเปลี่ยนสถานะกลับเป็น รออนุมัติ ใช่หรือไม่?')">
                                    <i class="ri-restart-line me-2"></i>เปลี่ยนสถานะเป็น รออนุมัติ
                                </button>
                            </div>
                        <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
                    </div>
                    ...
                    <div class="summary-section">
                        <div class="notes-card">
                            ...
                            <div class="form-group">
                                ...
                                <!--[if BLOCK]><![endif]--><?php if($quote_status === 'cancel' && !auth()->user()->hasAdminAccess()): ?>
                                    <small class="text-muted">
                                        <i class="ri-lock-line me-1"></i>หมายเหตุถูกล็อก - สามารถแก้ไขได้เฉพาะ Admin และ SA เท่านั้น
                                    </small>
                                <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
                            </div>
                        </div>
...
?>
        ...
        function showRejectReasonModal(quotationId, quoteNumber) {
            Swal.fire({
                title: 'ไม่อนุมัติใบเสนอราคา',
                html: `
                    <div style="text-align: left;">
                        <p style="margin-bottom: 10px;">เลขที่: <strong>${quoteNumber}</strong></p>
                        <label style="display: block; margin-bottom: 5px; font-weight: 600;">กรุณาระบุเหตุผล <span style="color: red;">*</span></label>
                        <textarea id="reject-reason" class="swal2-textarea" 
                                  placeholder="กรอกเหตุผลการไม่อนุมัติ..." 
                                  style="width: 100%; min-height: 100px; padding: 10px; border: 2px solid #e5e7eb; border-radius: 6px; font-size: 14px;"></textarea>
                    </div>
                `,
                icon: 'warning',
                ...
                preConfirm: () => {
                    ...
                    if (!reason || reason.trim() === '') {
                        Swal.showValidationMessage('กรุณาระบุเหตุผลการไม่อนุมัติ');
                        return false;
                    }
                    return reason;
                }
                ...
                if (result.isConfirmed && result.value) {
                    ...
                    if (livewireComponent) {
                        livewireComponent.call('rejectQuotation', quotationId, result.value);
                    }
                }
            });
        }
